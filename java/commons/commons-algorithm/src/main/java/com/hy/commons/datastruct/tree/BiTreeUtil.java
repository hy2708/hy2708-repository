...
public class BiTreeUtil {
	public static BiNode buildTreeByString(String tree,int index) {
		...
		node.left=buildTreeByString(tree,2*index+1);
		node.right=buildTreeByString(tree,2*index+2);
		return node;
	}
...
//		xianxuTraverse_FeiDiGui2(root);
	...
	public static void xianxuTraverse(BiNode root) {
		...
		xianxuTraverse(root.left);
		xianxuTraverse(root.right);
	}
	...
	public static void kuandu_xianxuTraverse(BiNode root,int cengShu,int[] arr) {
		...
		kuandu_xianxuTraverse(root.left, cengShu+1, arr);

		kuandu_xianxuTraverse(root.right, cengShu+1, arr);
