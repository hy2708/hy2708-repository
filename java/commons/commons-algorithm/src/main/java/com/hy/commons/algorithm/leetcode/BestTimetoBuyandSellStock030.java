...
public class BestTimetoBuyandSellStock030 {
	...
	public int maxProfit(int[] prices) {
        ...
        int iElemtMaxProfit=0;
        int[] xxmaxNumbers  = new int[pricesSize];
        xxmaxNumbers[pricesSize-1] = prices[pricesSize-1];
        //从后面向前遍历
        for(int i = pricesSize-2; i >= 0; i--){
            if( prices[i] < xxmaxNumbers[i] ){
            	xxmaxNumbers[i] = prices[i];
            }
